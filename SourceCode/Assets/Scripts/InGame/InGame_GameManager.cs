...
public class InGame_GameManager : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        */
        ...
        {
            ////////////// 종료 조건 Player와 Enemy의 거리가 X Y 축  +- 20 이하면 다시 실행하도록 한다.
            /*
            if((Math.Abs(mMap.mPlayers[0].mNodePositionXY.y - mMap.mPlayers[1].mNodePositionXY.y) < StaticVariables.sGameOverDistance
                && Math.Abs(mMap.mPlayers[0].mNodePositionXY.x - mMap.mPlayers[1].mNodePositionXY.x) < StaticVariables.sGameOverDistance) || mMap.mItems.Count <= 0
                || mMap.mPlayers[0].mCalculateTimes.Count == 6 || mMap.mPlayers[0].mGotItemsCount == 5) // TODO 테스트용  || mMap.mPlayers[0].mGotItems ==6 부분 지울 것/////////////////////////////
            {
                if (mMap.mPlayers[0].mCalculateTimes.Count == 6 && mMap.mPlayers[0].mGotItemsCount==5) //TODO JYW 임시용, 습득 아이템이 6개 이상이면 csv 저장
                {
