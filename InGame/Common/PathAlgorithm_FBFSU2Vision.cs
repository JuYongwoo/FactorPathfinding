...
public class PathAlgorithm_FBFSU2Vision : PathAlgorithm
{
    ...
    public override void preProcessing(MapData pMap, bool pIsMoving) // Main
    {
        if (!pIsMoving)
        {
            ...
            if (mNowMapID != pMap.mMapID)
            {
                ...
                mScoreBoard = StaticPathUtils.getScoreBoardwithBFSfromEnemiesStartPosition(pMap, mActorIndex); //시작 위치만 알기 때문에 시작 위치부터
            }

            if (mCount++ % 2 == 0) //탐색 2회 간격으로 safetyscore를 업데이트한다.
            {
                mScoreBoard = StaticPathUtils.substractScoreBoard(mScoreBoard);
            }
        }
    ...
    public override List<Vector2Int> getFindResult(MapData pMap, int pActorIndex) // Main
    {
        ...
        // 목표 설정
        List<Vector2Int> lGoals = new List<Vector2Int>(getVisionGoals(pMap, pActorIndex));

        //길찾기 시작
        List <Vector2Int> lItemsResults = StaticPathUtils.getPathwithBFS(pMap, pMap.mPlayers[mActorIndex].mNodePositionXY, lGoals, mScoreBoard);
